...
const CODBusinessModel = () => {
  ...
  const [parameters, setParameters] = useState({
    ...
    // Partners
    initialPartners: 4,
    saudiPartners: 1, // Saudi national (doesn't need visa)
    
    // KSA Company Setup Costs (one-time)
    misaLicense: 12000, // MISA Foreign Investment License
    commercialRegistration: 6000, // Commercial Registration (CR)
    municipalLicense: 3000, // Municipal License (Riyadh mid-range)
    logisticsLicense: 15000, // Services License for logistics aggregator
    documentationCosts: 4000, // Notarization, translation, attestation
    bankAccountSetup: 10000, // Bank account opening minimum deposit
    legalProfessionalFees: 25000, // Legal and business setup consultancy
    officeSetupDeposit: 20000, // Office rent deposit and basic setup
    
    // Visa and Work Permit Costs (from research)
    visaCost: 2000, // Business/Work visa cost (SAR 2,000 each - same for partners and employees)
    workPermitCost: 9600, // Annual work permit cost (SAR 9,600/year - same for partners and employees)
    iqamaRenewalCost: 650, // Annual Iqama renewal (SAR 650/year)
    
    // Applicability options (1: All Staff, 2: Sales Staff & Partners, 3: Partners Only, 4: No Staff)
    visaCostApplicability: 3,
    workPermitCostApplicability: 3, 
    iqamaRenewalApplicability: 3,
    
    // Annual License Renewals
    misaAnnualRenewal: 62000, // MISA annual renewal (SAR 62,000/year)
    commercialRegistrationRenewal: 1200, // CR annual renewal (SAR 1,200/year)
    
    // Revenue rates
    standardCODRate: 1.98,
    premiumCODRate: 1.98,
    ...
    // COD Working Capital Parameters
    avgOrderValue: 60,
    ...
    basePartnerSalary: 0, // Assumed monthly salary for partners
    opsStaffPer1000Orders: 2.5,
    salesStaffPer1000Orders: 3.2,
    minOpsStaff: 1,
    minSalesStaff: 1,
  ...
  const calculateStaffForCost = (applicability, opsStaff, salesStaff, nonSaudiPartners) => {
  ...
  // Calculate total KSA setup costs
  const totalSetupCosts = useMemo(() => {
  ...
  const handleApplicabilityChange = (key, value) => {
    setParameters(prev => ({
      ...prev,
  ...
  const chartData = projections.slice(1, 37);
  ...
  const revenueBreakdown = [
    { name: 'Standard COD', value: projections[36].standardRevenue, color: '#0088FE' },
    { name: 'Premium COD', value: projections[36].premiumRevenue, color: '#00C49F' },
    { name: 'Warehousing', value: projections[36].warehousingRevenue, color: '#FFBB28' }
  ];
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50 p-6">
      <div className="max-w-7xl mx-auto">
        {/* Header */}
        <div className="bg-gradient-to-r from-orange-500 to-red-600 text-white p-8 rounded-2xl mb-8 shadow-2xl">
          <h1 className="text-4xl font-bold mb-4">🚀 COD Business Financial Model</h1>
          <h2 className="text-2xl font-semibold mb-4">📊 400,000 SAR Investment - Saudi Arabia Market</h2>
          <p className="text-xl opacity-90">Interactive Financial Projections with Partners, Ops & Sales Staff Scaling</p>
          
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mt-6">
            <div className="bg-white/10 rounded-xl p-4">
              <div className="text-sm opacity-80">Initial Investment</div>
              <div className="text-2xl font-bold">{formatCurrency(parameters.initialInvestment)}</div>
            </div>
            <div className="bg-white/10 rounded-xl p-4">
              <div className="text-sm opacity-80">Break-Even Month</div>
              <div className="text-2xl font-bold">Month {keyMetrics.breakEvenMonth || 'N/A'}</div>
            </div>
            <div className="bg-white/10 rounded-xl p-4">
              <div className="text-sm opacity-80">36-Month ROI</div>
              <div className="text-2xl font-bold">{keyMetrics.roi.toLocaleString()}%</div>
            </div>
            <div className="bg-white/10 rounded-xl p-4">
              <div className="text-sm opacity-80">KSA Setup Cost</div>
              <div className="text-2xl font-bold">{formatCurrency(totalSetupCosts)}</div>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="bg-white rounded-2xl shadow-xl p-6">
            <h2 className="text-2xl font-bold mb-6 text-gray-800">🏢 KSA Setup & Renewal Costs</h2>
            ...
            <div className="mb-6">
              ...
              <div className="space-y-3">
                <div className="flex justify-between items-center p-2 bg-gray-50 rounded">
                  <span className="text-sm">MISA License</span>
                  <span className="font-semibold">{formatCurrency(parameters.misaLicense)}</span>
                </div>
                <div className="flex justify-between items-center p-2 bg-gray-50 rounded">
                  <span className="text-sm">Commercial Registration</span>
                  <span className="font-semibold">{formatCurrency(parameters.commercialRegistration)}</span>
                </div>
                <div className="flex justify-between items-center p-2 bg-gray-50 rounded">
                  <span className="text-sm">Municipal License</span>
                  <span className="font-semibold">{formatCurrency(parameters.municipalLicense)}</span>
                </div>
                <div className="flex justify-between items-center p-2 bg-gray-50 rounded">
                  <span className="text-sm">Logistics License</span>
                  <span className="font-semibold">{formatCurrency(parameters.logisticsLicense)}</span>
                </div>
                <div className="flex justify-between items-center p-2 bg-gray-50 rounded">
                  <span className="text-sm">Documentation Costs</span>
                  <span className="font-semibold">{formatCurrency(parameters.documentationCosts)}</span>
                </div>
                <div className="flex justify-between items-center p-2 bg-gray-50 rounded">
                  <span className="text-sm">Bank Account Setup</span>
                  <span className="font-semibold">{formatCurrency(parameters.bankAccountSetup)}</span>
                </div>
                <div className="flex justify-between items-center p-2 bg-gray-50 rounded">
                  <span className="text-sm">Legal & Professional Fees</span>
                  <span className="font-semibold">{formatCurrency(parameters.legalProfessionalFees)}</span>
                </div>
                <div className="flex justify-between items-center p-2 bg-gray-50 rounded">
                  <span className="text-sm">Office Setup Deposit</span>
                  <span className="font-semibold">{formatCurrency(parameters.officeSetupDeposit)}</span>
                </div>
                <div className="flex justify-between items-center p-2 bg-gray-50 rounded">
                  <span className="text-sm">{parameters.initialPartners - parameters.saudiPartners} Partner Visas</span>
                  <span className="font-semibold">{formatCurrency((parameters.initialPartners - parameters.saudiPartners) * parameters.visaCost)}</span>
                </div>
                <div className="flex justify-between items-center p-2 bg-gray-50 rounded">
                  <span className="text-sm">{parameters.initialPartners - parameters.saudiPartners} Partner Work Permits</span>
                  <span className="font-semibold">{formatCurrency((parameters.initialPartners - parameters.saudiPartners) * parameters.workPermitCost)}</span>
                </div>
                <div className="border-t-2 border-orange-500 pt-2 mt-2">
                  <div className="flex justify-between items-center p-2 bg-orange-100 rounded font-bold">
                    <span>Total Setup Cost</span>
                    <span className="text-orange-600">{formatCurrency(totalSetupCosts)}</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div>
              ...
              <div className="space-y-3">
                <div className="flex justify-between items-center p-2 bg-red-50 rounded">
                  <span className="text-sm">MISA License Renewal</span>
                  <span className="font-semibold text-red-600">{formatCurrency(parameters.misaAnnualRenewal)}</span>
                </div>
                <div className="flex justify-between items-center p-2 bg-red-50 rounded">
                  <span className="text-sm">Commercial Registration Renewal</span>
                  <span className="font-semibold text-red-600">{formatCurrency(parameters.commercialRegistrationRenewal)}</span>
                </div>
                <div className="flex justify-between items-center p-2 bg-red-50 rounded">
                  <span className="text-sm">Work Permits (per staff)</span>
                  <span className="font-semibold text-red-600">{formatCurrency(parameters.workPermitCost)}</span>
                </div>
                <div className="flex justify-between items-center p-2 bg-red-50 rounded">
                  <span className="text-sm">Iqama Renewals (per staff)</span>
                  <span className="font-semibold text-red-600">{formatCurrency(parameters.iqamaRenewalCost)}</span>
                </div>
                <div className="border-t-2 border-red-500 pt-2 mt-2">
                  <div className="flex justify-between items-center p-2 bg-red-100 rounded font-bold">
                    <span>Fixed Annual Cost</span>
                    <span className="text-red-600">{formatCurrency(parameters.misaAnnualRenewal + parameters.commercialRegistrationRenewal)}</span>
                  </div>
                  <div className="flex justify-between items-center p-2 bg-red-100 rounded mt-2">
                    <span>Annual Staff Costs (per staff)</span>
                    <span className="text-red-600">{formatCurrency(parameters.workPermitCost + parameters.iqamaRenewalCost)}</span>
                  </div>
          ...
          {/* Parameters Panel */}
          <div className="bg-white rounded-2xl shadow-xl p-6">
            <h2 className="text-2xl font-bold mb-6 text-gray-800">📊 Business Parameters</h2>
            ...
            <div className="space-y-6">
              ...
              <div>
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => handleParameterChange('initialInvestment', e.target.value)}
              ...
              <div>
                ...
                <div className="space-y-3">
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Total Partners</label>
                    <input
                      type="number"
                      value={parameters.initialPartners}
                      onChange={(e) => handleParameterChange('initialPartners', e.target.value)}
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500"
                    />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Saudi Partners (no visa needed)</label>
                    <input
                      type="number"
                      value={parameters.saudiPartners}
                      onChange={(e) => handleParameterChange('saudiPartners', e.target.value)}
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500"
                    />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Partner Monthly Salary (SAR)</label>
                    <input
                      type="number"
                      value={parameters.basePartnerSalary}
                      onChange={(e) => handleParameterChange('basePartnerSalary', e.target.value)}
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500"
                    />
                  </div>
                </div>
                ...
                <div className="space-y-3">
                  <div>
                    ...
                    <input
                      type="number"
                      step="0.01"
                      value={parameters.standardCODRate}
                      onChange={(e) => handleParameterChange('standardCODRate', e.target.value)}
                  ...
                  <div>
                    ...
                    <input
                      type="number"
                      step="0.01"
                      value={parameters.premiumCODRate}
                      onChange={(e) => handleParameterChange('premiumCODRate', e.target.value)}
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500"
                    />
                  </div>
                  <div>
                    ...
                    <input
                      type="number"
                      step="0.01"
                      value={parameters.warehousingRate}
                      onChange={(e) => handleParameterChange('warehousingRate', e.target.value)}
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500"
                    />
                  </div>
                </div>
              ...
              <div>
                ...
                <div className="space-y-3">
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Initial Orders/Day</label>
                    <input
                      type="number"
                      value={parameters.initialOrdersPerDay}
                      onChange={(e) => handleParameterChange('initialOrdersPerDay', e.target.value)}
                    ...
                    <div className="space-y-2">
                      <label className="flex items-center">
                        <input
                          ...
                          onChange={() => handleParameterChange('useFixedGrowthPattern', true)}
                          className="mr-2"
                        />
                        <span className="text-sm">Use Excel Growth Pattern</span>
                      </label>
                      <label className="flex items-center">
                        <input
                          type="radio"
                          name="growthPattern"
                          checked={!parameters.useFixedGrowthPattern}
                          onChange={() => handleParameterChange('useFixedGrowthPattern', false)}
                          className="mr-2"
                        />
                        <span className="text-sm">Use Growth Rate</span>
                      </label>
                    </div>
                  </div>
                  {!parameters.useFixedGrowthPattern && (
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => handleParameterChange('monthlyGrowthRate', e.target.value)}
                        className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500"
                      />
                      <p className="text-xs text-gray-500 mt-1">
                        Current: {parameters.monthlyGrowthRate}% monthly = {((Math.pow(1 + parameters.monthlyGrowthRate/100, 12) - 1) * 100).toFixed(1)}% annually
                      </p>
                    </div>
                  )}
                </div>
              ...
              <div>
                ...
                <div className="space-y-3">
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Average Order Value (SAR)</label>
                    <input
                      type="number"
                      value={parameters.avgOrderValue}
                      onChange={(e) => handleParameterChange('avgOrderValue', e.target.value)}
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-500"
                    />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">3PL Payment Days</label>
                    <input
                      type="number"
                      value={parameters.threePlPaymentDays}
                      onChange={(e) => handleParameterChange('threePlPaymentDays', e.target.value)}
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-500"
                    />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Premium Settlement Days</label>
                    <input
                      type="number"
                      value={parameters.premiumSettlementDays}
                      onChange={(e) => handleParameterChange('premiumSettlementDays', e.target.value)}
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-500"
                    />
                  </div>
                </div>
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    <div className="flex justify-between items-center">
                      <label className="block text-sm font-medium text-gray-700 mb-1">Visa Cost (SAR/person)</label>
                      <input
                        type="number"
                        value={parameters.visaCost}
                        onChange={(e) => handleParameterChange('visaCost', e.target.value)}
                        className="w-1/3 px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-500"
                      />
                    </div>
                    <div className="mt-2 border border-gray-200 rounded-lg p-2">
                      ...
                      <div className="space-y-2">
                        <label className="flex items-center">
                          <input
                            type="radio"
                            name="visaCostApplicability"
                            value="1"
                            checked={parameters.visaCostApplicability === 1}
                            onChange={(e) => handleApplicabilityChange('visaCostApplicability', e.target.value)}
                            className="mr-2"
                          />
                        ...
                        <label className="flex items-center">
                          <input
                            ...
                            name="visaCostApplicability"
                            value="2"
                            checked={parameters.visaCostApplicability === 2}
                            onChange={(e) => handleApplicabilityChange('visaCostApplicability', e.target.value)}
                            className="mr-2"
                          />
                          <span className="text-sm">Sales Staff & Partners</span>
                        </label>
                        <label className="flex items-center">
                          <input
                            type="radio"
                            name="visaCostApplicability"
                            value="3"
                            checked={parameters.visaCostApplicability === 3}
                            onChange={(e) => handleApplicabilityChange('visaCostApplicability', e.target.value)}
                            className="mr-2"
                          />
                        ...
                        <label className="flex items-center">
                          <input
                            ...
                            name="visaCostApplicability"
                            value="4"
                            checked={parameters.visaCostApplicability === 4}
                            onChange={(e) => handleApplicabilityChange('visaCostApplicability', e.target.value)}
                            className="mr-2"
                          />
                          <span className="text-sm">No Staff</span>
                        </label>
                      </div>
                    </div>
                  </div>
                  ...
                  <div>
                    <div className="flex justify-between items-center">
                      <label className="block text-sm font-medium text-gray-700 mb-1">Work Permit Cost (SAR/year)</label>
                      <input
                        type="number"
                        value={parameters.workPermitCost}
                        onChange={(e) => handleParameterChange('workPermitCost', e.target.value)}
                        className="w-1/3 px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-500"
                      />
                    </div>
                    <div className="mt-2 border border-gray-200 rounded-lg p-2">
                      ...
                      <div className="space-y-2">
                        <label className="flex items-center">
                          <input
                            type="radio"
                            name="workPermitCostApplicability"
                            value="1"
                            checked={parameters.workPermitCostApplicability === 1}
                            onChange={(e) => handleApplicabilityChange('workPermitCostApplicability', e.target.value)}
                            className="mr-2"
                          />
                          <span className="text-sm">All Staff</span>
                        </label>
                        <label className="flex items-center">
                          <input
                            ...
                            name="workPermitCostApplicability"
                            value="2"
                            checked={parameters.workPermitCostApplicability === 2}
                            onChange={(e) => handleApplicabilityChange('workPermitCostApplicability', e.target.value)}
                            className="mr-2"
                          />
                          <span className="text-sm">Sales Staff & Partners</span>
                        ...
                        <label className="flex items-center">
                          <input
                            ...
                            name="workPermitCostApplicability"
                            value="3"
                            checked={parameters.workPermitCostApplicability === 3}
                            onChange={(e) => handleApplicabilityChange('workPermitCostApplicability', e.target.value)}
                            className="mr-2"
                          />
                          <span className="text-sm">Partners Only</span>
                        ...
                        <label className="flex items-center">
                          <input
                            ...
                            name="workPermitCostApplicability"
                            value="4"
                            checked={parameters.workPermitCostApplicability === 4}
                            onChange={(e) => handleApplicabilityChange('workPermitCostApplicability', e.target.value)}
                            className="mr-2"
                          />
                          <span className="text-sm">No Staff</span>
                  ...
                  <div>
                    <div className="flex justify-between items-center">
                      <label className="block text-sm font-medium text-gray-700 mb-1">Iqama Renewal Cost (SAR/year)</label>
                      <input
                        type="number"
                        value={parameters.iqamaRenewalCost}
                        onChange={(e) => handleParameterChange('iqamaRenewalCost', e.target.value)}
                        className="w-1/3 px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-500"
                      />
                    </div>
                    <div className="mt-2 border border-gray-200 rounded-lg p-2">
                      ...
                      <div className="space-y-2">
                        <label className="flex items-center">
                          <input
                            type="radio"
                            name="iqamaRenewalApplicability"
                            value="1"
                            checked={parameters.iqamaRenewalApplicability === 1}
                            onChange={(e) => handleApplicabilityChange('iqamaRenewalApplicability', e.target.value)}
                            className="mr-2"
                          />
                          <span className="text-sm">All Staff</span>
                        </label>
                        <label className="flex items-center">
                          <input
                            ...
                            name="iqamaRenewalApplicability"
                            value="2"
                            checked={parameters.iqamaRenewalApplicability === 2}
                            onChange={(e) => handleApplicabilityChange('iqamaRenewalApplicability', e.target.value)}
                            className="mr-2"
                          />
                          <span className="text-sm">Sales Staff & Partners</span>
                        ...
                        <label className="flex items-center">
                          <input
                            ...
                            name="iqamaRenewalApplicability"
                            value="3"
                            checked={parameters.iqamaRenewalApplicability === 3}
                            onChange={(e) => handleApplicabilityChange('iqamaRenewalApplicability', e.target.value)}
                            className="mr-2"
                          />
                          <span className="text-sm">Partners Only</span>
                        ...
                        <label className="flex items-center">
                          <input
                            ...
                            name="iqamaRenewalApplicability"
                            value="4"
                            checked={parameters.iqamaRenewalApplicability === 4}
                            onChange={(e) => handleApplicabilityChange('iqamaRenewalApplicability', e.target.value)}
                            className="mr-2"
                          />
                          <span className="text-sm">No Staff</span>
                  ...
                  <p className="text-xs text-gray-500 mt-2">
                    Saudi nationals are automatically excluded from all visa-related costs
                  </p>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="space-y-3">
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Ops Staff Salary (SAR/month)</label>
                    <input
                      type="number"
                      value={parameters.baseOpsStaffSalary}
                      onChange={(e) => handleParameterChange('baseOpsStaffSalary', e.target.value)}
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500"
                    />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Sales Staff Salary (SAR/month)</label>
                    <input
                      type="number"
                      value={parameters.baseSalesStaffSalary}
                      onChange={(e) => handleParameterChange('baseSalesStaffSalary', e.target.value)}
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500"
                    />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Ops Staff per 1000 Orders/Day</label>
                    <input
                      type="number"
                      step="0.1"
                      value={parameters.opsStaffPer1000Orders}
                      onChange={(e) => handleParameterChange('opsStaffPer1000Orders', e.target.value)}
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500"
                    />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Sales Staff per 1000 Orders/Day</label>
                    <input
                      type="number"
                      step="0.1"
                      value={parameters.salesStaffPer1000Orders}
                      onChange={(e) => handleParameterChange('salesStaffPer1000Orders', e.target.value)}
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500"
                    />
                  </div>
                </div>
          ...
          <div className="space-y-8">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div className="bg-gradient-to-br from-blue-500 to-blue-600 text-white rounded-xl p-4">
                <h4 className="text-sm font-medium opacity-90">Total Revenue (36m)</h4>
                <div className="text-lg font-bold">{formatCurrency(keyMetrics.totalRevenue36Months)}</div>
              </div>
              <div className="bg-gradient-to-br from-green-500 to-green-600 text-white rounded-xl p-4">
                <h4 className="text-sm font-medium opacity-90">Total Profit (36m)</h4>
                <div className="text-lg font-bold">{formatCurrency(keyMetrics.totalProfit36Months)}</div>
              </div>
              <div className="bg-gradient-to-br from-purple-500 to-purple-600 text-white rounded-xl p-4">
                <h4 className="text-sm font-medium opacity-90">Peak Monthly Profit</h4>
                <div className="text-lg font-bold">{formatCurrency(keyMetrics.peakMonthlyProfit)}</div>
              </div>
              <div className="bg-gradient-to-br from-orange-500 to-orange-600 text-white rounded-xl p-4">
                <h4 className="text-sm font-medium opacity-90">Final Orders/Day</h4>
                <div className="text-lg font-bold">{formatNumber(keyMetrics.finalOrdersPerDay)}</div>
              </div>
            </div>

            {/* Revenue & Profit Chart */}
            <div className="bg-white rounded-2xl shadow-xl p-6">
              <h3 className="text-xl font-bold mb-4 text-gray-800">📈 Monthly Revenue vs Profit</h3>
            ...
            <div className="bg-white rounded-2xl shadow-xl p-6">
              <h3 className="text-xl font-bold mb-4 text-gray-800">💰 Cash Flow vs Working Capital Impact</h3>
              <ResponsiveContainer width="100%" height={300}>
                <ComposedChart data={chartData}>
                  <CartesianGrid strokeDasharray="3 3" />
                  <XAxis dataKey="month" />
            ...
            <div className="bg-white rounded-2xl shadow-xl p-6">
              <h3 className="text-xl font-bold mb-4 text-gray-800">👥 Staff Growth: Partners, Ops & Sales</h3>
              <ResponsiveContainer width="100%" height={300}>
        ...
        <div className="mt-8 bg-white rounded-2xl shadow-xl p-6">
          <h3 className="text-2xl font-bold mb-6 text-gray-800">🎯 Key Milestones</h3>
                ...
                {keyMilestones.map((milestone, index) => (
                  <tr 
                    ...
                    className={`
                      ...
                      ${milestone.monthlyProfit > 0 ? 'border-l-4 border-green-400' : ''}
                      ${milestone.month === keyMetrics.breakEvenMonth ? 'bg-green-100' : ''}
                    `}
                  >
                    ...
                    <td className={`px-3 py-3 text-center font-bold ${milestone.monthlyProfit >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                      {formatNumber(milestone.monthlyProfit)}
                    </td>
                    ...
                    <td className="px-3 py-3 text-center font-semibold text-orange-600">
                      {formatNumber(milestone.cashPosition)}
                    </td>
                  </tr>
                ))}
              </tbody>
        ...
        <div className="mt-8 bg-white rounded-2xl shadow-xl p-6">
          <h3 className="text-2xl font-bold mb-6 text-gray-800">📊 Complete Monthly Projections (All 36 Months)</h3>
              ...
              <tbody>
                {projections.map((month, index) => (
                  <tr 
                    ...
                    className={`
                      ${index % 2 === 0 ? 'bg-gray-50' : 'bg-white'}
                      ${month.month === 0 ? 'bg-orange-100' : ''}
                      ${month.cumulativeProfit > 0 && (index === 0 || projections[index - 1]?.cumulativeProfit <= 0) ? 'bg-green-100' : ''}
                      ${month.month === 5 || month.month === 10 || month.month === 13 || month.month === 25 ? 'bg-blue-50' : ''}
                    `}
                  >
                    ...
                    <td className={`px-2 py-2 text-center font-bold ${month.monthlyProfit >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                      {month.monthlyProfit >= 0 ? formatNumber(month.monthlyProfit) : `(${formatNumber(Math.abs(month.monthlyProfit))})`}
                    </td>
                    <td className="px-2 py-2 text-center font-semibold text-orange-600">
                      {formatNumber(month.codWorkingCapital)}
                    </td>
                    <td className="px-2 py-2 text-center text-orange-700">
                      {formatNumber(month.premiumCODSettlement)}
                    </td>
                    <td className={`px-2 py-2 text-center font-bold ${month.cumulativeProfit >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                      {month.cumulativeProfit >= 0 ? formatNumber(month.cumulativeProfit) : `(${formatNumber(Math.abs(month.cumulativeProfit))})`}
                    </td>
                    <td className="px-2 py-2 text-center font-semibold text-purple-600">
                      {formatNumber(month.cashPosition)}
                    </td>
                  </tr>
                ))}
              </tbody>
        ...
        <div className="mt-8 bg-white rounded-2xl shadow-xl p-6">
          <h3 className="text-2xl font-bold mb-6 text-gray-800">📋 Business Summary</h3>
          
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div className="space-y-4">
              <div className="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
                <span className="font-medium">Initial Investment</span>
                <span className="font-bold text-orange-600">{formatCurrency(parameters.initialInvestment)}</span>
              </div>
              <div className="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
                <span className="font-medium">Final Cash Position</span>
                <span className="font-bold text-green-600">{formatCurrency(keyMetrics.finalCashPosition)}</span>
              </div>
              <div className="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
                <span className="font-medium">Total Revenue (36 months)</span>
                <span className="font-bold text-blue-600">{formatCurrency(keyMetrics.totalRevenue36Months)}</span>
              </div>
              <div className="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
                <span className="font-medium">Total Profit (36 months)</span>
                <span className="font-bold text-green-600">{formatCurrency(keyMetrics.totalProfit36Months)}</span>
              </div>
              <div className="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
                <span className="font-medium">ROI (%)</span>
                <span className="font-bold text-purple-600">{keyMetrics.roi.toLocaleString()}%</span>
              </div>
            </div>
            
            <div className="space-y-4">
              <div className="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
                <span className="font-medium">Peak Monthly Profit</span>
                <span className="font-bold text-green-600">{formatCurrency(keyMetrics.peakMonthlyProfit)}</span>
              </div>
              <div className="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
                <span className="font-medium">Break-even Month</span>
                <span className="font-bold text-blue-600">Month {keyMetrics.breakEvenMonth}</span>
              </div>
              <div className="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
                <span className="font-medium">Final Partners</span>
                <span className="font-bold text-red-600">{keyMetrics.finalPartners}</span>
              </div>
              <div className="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
                <span className="font-medium">Final Staff (Partners + Ops + Sales)</span>
                <span className="font-bold text-purple-600">{keyMetrics.finalTotalStaff}</span>
              </div>
              <div className="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
                <span className="font-medium">Final Orders Per Day</span>
                <span className="font-bold text-orange-600">{formatNumber(keyMetrics.finalOrdersPerDay)}</span>
              </div>
            </div>
          </div>
        </div>
...
export default CODBusinessModel;